    ...
    if (Array.isArray(body)) {
      ...
      for (const cmd of body) {
        ...
        if (op === 'SET') {
          ...
          const e = _store.get(cmd[1]);
          if (!e || e.expiresAt < Date.now()) { _store.delete(cmd[1]); results.push(null); }
...
async function main(): Promise<void> {
  const oauthState: any = await import('../src/lib/oauth-state.ts');
  ...
  const nonce1 = await mintOAuthState(orgId, userId);
  ...
  const freshNonce = await mintOAuthState(orgId, userId);
  const c7b = await consumeOAuthState(freshNonce, { orgId, userId: 'different-user' });
  ...
  const ok3 = await consumeOAuthState(
    await mintOAuthState('org_xyz_999', 'user_abc_999'),
