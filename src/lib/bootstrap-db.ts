...
CREATE TABLE IF NOT EXISTS sms_consent_events (id text PRIMARY KEY, org_id text REFERENCES organizations(id) ON DELETE CASCADE, customer_id text REFERENCES customers(id) ON DELETE SET NULL, phone text NOT NULL, event_type text NOT NULL, message_text text, twilio_sid text, created_at timestamptz NOT NULL DEFAULT now());
