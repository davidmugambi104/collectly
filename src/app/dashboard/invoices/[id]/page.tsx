...
export default async function InvoiceDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const runs = await db
    .select()
    .from(dunningRuns)
    .where(eq(dunningRuns.invoiceId, id))
    ...
    <AppShell title={`Invoice ${invoice.number}`} subtitle={`${customer.name} · ${formatCurrency(balance, invoice.currency)} ${isOverdue ? 'overdue' : 'open'}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="card">
            <h2 className="font-semibold text-ink-900">Payment portal</h2>
            <p className="mt-1 text-xs text-ink-600">Customers click this link to pay online.</p>
            <div className="mt-3 rounded-lg border border-ink-200 bg-ink-50 p-2 text-xs font-mono break-all">{portalUrl}</div>
