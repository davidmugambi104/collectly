    ...
    return (
      <div className="card">
        ...
        {recommendedAction && (
          <div className="mt-4 rounded-lg border border-brand-200 bg-brand-50/50 p-3 flex items-start gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              <div className="app-meta text-brand-900">AI recommendation</div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <button onClick={() => setOpen(true)} className="btn-brand btn-sm">
            <Sparkles className="h-3.5 w-3.5" />
            {daysOverdue > 0 ? `Send a ${tone} reminder` : 'Send a reminder'}
          </button>
          {recommendedChannel === 'phone' && phone && (
            <a href={`tel:${phone}`} className="btn-secondary btn-sm">
              <Phone className="h-3.5 w-3.5" />Call customer
            </a>
          )}
          <a href="/dashboard/dunning/sequence" className="btn-ghost btn-sm">Manage sequences →</a>
        </div>
      </div>
    );
  }
