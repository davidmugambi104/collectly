...
Updated: 2026-09-08 11:03 EAT (08:03 UTC) by `collectly-launch-executor` cron (Tue, daily)

> **HEADLINE (2026-09-08 11:03 EAT):** **🚨🚨 GATE PULLBACK DAY 6 — `pullback/cap=30`, bounce rate SURGED to 31.40% (65/207)** —
> pipeline THROTTLED. Fresh snapshot (Sep 8 08:03 UTC):
> **207 sends, 65 bounces, 31.40% bounce rate** — UP from 18.01% on Sep 7.
> MASSIVE bounce spike: bounces 29→65 (+36 new bounces), sends 161→207 (+46 new sends).
> **57 of 65 bounces are `hello@` pattern addresses** — enrichment source quality is the root cause.
> Bounces from Sep 2 OSM batch are STILL in the 7d window (aging out ~Sep 9).
> Seed-inbox: 6/163 total, 156 awaiting folder report. CRM: **324 rows** (up from 272),
> **1 malformed** (DOWN from 115 — reconciliation now using live outreach-log.csv as canonical), 323 clean, 306 sent, 17 send_failed. Rebuild NOT paused.
> Experiments: **A=77/1/1.30% (weight=1.0).** **B=77/0/0%, C=77/0/0%, D=76/0/0%** —
> **ALL B/C/D PAST KILL THRESHOLD (50+ sends, 0% reply).** 0/230 combined for B/C/D. 1 reply in 307 lifetime sends (0.33% overall).
> Rate: 24 sends on Sep 7. 1 new git commit since Sep 7 (`9b63198` checklist refresh). **48 days post-launch. Operational, not P0.**
>
