...
Updated: 2026-09-09 11:13 EAT (08:13 UTC) by `collectly-launch-executor` cron (Wed, daily)

> **HEADLINE (2026-09-09 11:13 EAT):** **🚨🚨 GATE PULLBACK DAY 7 — `pullback/cap=30`, bounce rate JUMPED to 34.36% (67/195)** —
> pipeline THROTTLED. Fresh snapshot (Sep 9 08:13 UTC):
> **195 sends, 67 bounces, 34.36% bounce rate** — UP from 31.40% on Sep 8.
> Bounce rate still CLIMBING: bounces 65→67 (+2), sends 207→195 (-12, 7d window shrinking).
> **`hello@` pattern addresses still dominate bounces.** Enrichment source quality remains the root cause.
> Bounces from Sep 2 OSM batch should start aging out TODAY (Sep 9, 7d after Sep 2 sends).
> Seed-inbox: 6/167 total, 160 awaiting folder report. CRM: **323 rows** (down from 324),
> **1 malformed** (stable), 293 prospect states emitted, 17 dup-touch flags. Rebuild NOT paused.
> Experiments: **A=77/1/1.30% (w=1.0).** **B=77/0/0%, C=77/0/0%, D=76/0/0%** —
> **ALL variants PAST KILL THRESHOLD.** 0/230 combined for B/C/D. 1 reply in 307 lifetime sends (0.33% overall).
> Rate: 0 sends today (Sep 9). 1 new git commit since Sep 8 (`5050bae` briefing). **49 days post-launch. Operational, not P0.**
>
...
### Today's refresh (2026-09-09 11:13 EAT, Wed — daily)

- Re-read `outreach/data/gate-status.json` (08:13 UTC today — **FRESH, 0.00h old**), `outreach/data/deliverability-snapshots/resend-7d-summary-2026-09-09.json` (08:13 UTC today — **FRESH, 0.00h old**), `outreach/data/deliverability-status.json` (08:13 UTC today — **FRESH**), `outreach/data/crm-reconciliation-2026-09-09.json` (08:13 UTC today — **FRESH**), `outreach/data/experiment-status.json` (current), `outreach/data/rate-usage.json` (Sep 9: 0 sends), `decisions.md`, `risks.md`.
- P0 launch state: **unchanged from 2026-08-04** — all P0 items closed.
  Launch happened 2026-07-22; we are now **49 days post-launch**.
  **No P0 launch escalation active.**
- **🚨🚨 GATE-STATE DELTA VS Sep 8 11:03 EAT REFRESH (~24h ago):**
  - `gate-status.json` (08:13 UTC today — **FRESH, 0.00h old**):
    **`gate: pullback`**, `cap: 30`, `bounce_rate: 34.36%` (67/195),
    `spam: 0.0%`, `deliverability_status: conditional_pass`.
    **GATE REMAINS IN PULLBACK — Day 7.** Pipeline is THROTTLED to 30 sends/day.
  - **Fresh live API snapshot** (`resend-7d-summary-2026-09-09.json`, 08:13 UTC today — **FRESH, 0.00h old**):
    **195 sends, 67 bounces**, **34.36% bounce rate**, 0 spam, 65.64% delivery.
    **🚨🚨 BOUNCE RATE CLIMBED AGAIN: 31.40% → 34.36%.** Sends shrank 207→195 (-12, 7d window
    rolling) while bounces grew 65→67 (+2). Smaller denominator + more bounces = higher rate.
    **`hello@` pattern addresses still dominate the bounce list.** Enrichment source quality
    is the root cause. Bounces from Sep 2 OSM batch should start aging out TODAY (Sep 9).
    **This is the worst bounce rate since launch — 3rd consecutive day of worsening.**
  - `deliverability-status.json` (08:13 UTC today — **FRESH**): `status: conditional_pass`,
    `bounce_rate: 34.36%` (67/195). Seed-inbox: **6/167 total** (up from 6/163),
    **160 awaiting** folder report (up from 156), 0 Promotions, 0 Spam, 1 failed.
    Still need 2 more Primary flips to graduate to `pass`.
  - CRM reconciliation (08:13 UTC today — **FRESH**): **323 rows** (DOWN from 324),
    **1 malformed** (stable), **293 prospect states emitted** (up from 306 — slight change
    due to 7d window rolling), 17 dup-touch flags (unchanged). `rebuild_paused_reason` =
    `duplicate_touch_flags=17; unresolved_malformed=1`. **CRM is healthy.**
  - Experiment status: **A=77/1/1.30% (w=1.0).**
    **B=77/0/0% (w=1.0), C=77/0/0% (w=1.0), D=76/0/0% (w=1.0)** —
    **ALL variants PAST KILL THRESHOLD (50+ sends, 0% reply for B/C/D; A below 2% kill).**
    0/230 combined for B/C/D. 1 reply in 307 lifetime sends (0.33% overall).
    **No working copy remains. All variants should be killed per experiment rules.**
  - `rate-usage.json` (Sep 9): 0 sends, 0 API calls so far today.
    Pipeline active under pullback cap but no sends yet.
  - Git: **1 new commit** since Sep 8: `5050bae` (briefing: daily briefing 2026-09-08).
  - Dependency audit: Latest is Sep 7 (`security/dependency-audit-2026-09-07.md`).
    1c/29h/20m/6l = 56 total. `node-tar` critical still no upstream fix.
  - Briefing: **Sep 8 briefing exists** (`5050bae`). No Sep 9 briefing yet.
  - KPI: Latest KPI is `2026-09-07.md` — no Sep 8/9 KPI yet.
- **Key changes since Sep 8:**
  1. **🚨🚨 BOUNCE RATE CLIMBED 31.40% → 34.36%.** 3rd consecutive day of worsening.
     Sends shrank 207→195 (-12) while bounces grew 65→67 (+2). Worst bounce rate since launch.
     **Bounces from Sep 2 should start aging out TODAY (Sep 9) — expect significant drop.**
  2. CRM rows 324→323 (stable), malformed 1 (stable). 293 states emitted.
  3. Experiments unchanged — all variants still past kill threshold, no new copy.
  4. Seed-inbox grew 163→167 (+4), no new Primary flips. 160 awaiting.
  5. 0 sends today (Sep 9) — pipeline throttled.
  6. 1 new git commit (Sep 8 briefing).
- **Carry-over escalation candidates (updated from Sep 8 refresh):**
  1. **🚨🚨 CRITICAL — Gate `pullback/cap=30`, 34.36% bounce (67/195). Day 7.**
     Bounce rate has WORSENED for 3 consecutive days. **48h+ ESCALATION — ACTIVE.**
     **Davie action needed URGENTLY:** (a) suppress ALL 67 bounced addresses in
     `suppression.csv`, (b) STOP sending to `hello@` pattern addresses immediately,
     (c) pause OSM-sourced prospect sends until enrichment quality is verified,
     (d) investigate/switch Apify enrichment source — `hello@` pattern is clearly
     generating invalid addresses, (e) fix `deliverability_gate.py` to read live API directly.
     **Bounces from Sep 2 should age out TODAY (Sep 9) — rate should drop significantly.**
     **THIS IS A 72h+ ESCALATION ITEM — bounce rate has worsened for 3 consecutive days.**
  2. **🚨 CRITICAL — ALL experiment variants past kill threshold.**
     A=77/1/1.30% (below 2% kill), B=77/0/0%, C=77/0/0%, D=76/0/0%.
     **No working copy remains. Davie must prepare new copy variants immediately.**
     1 reply in 307 lifetime sends (0.33% overall).
  3. **Gate-bypass event 2026-08-09 — now ~31.4 days old, ~29 days past
     "needs decision" threshold.** 17 prospect IDs on T2 pause.
  4. Seed-inbox: 6/167 total, 160 awaiting. Need 2 more Primary flips
     to graduate to `pass`. Follow-ups remain paused (7th day).
  5. Dependency vulns: 1c/29h/20m/6l = 56. `postcss` path traversal (high) new.
     `node-tar` critical still no upstream fix.
  6. Dev/prod isolation gap — open since 2026-08-04.
  7. Lana Hill reply — now **41 days** in `human_review_priority`,
     27 days past hard escalation threshold.
  8. `folder-nudge-DRAFTS-2026-08-11.md` — now 29 days old.
  9. imapclient install + IMAP poller — 49 days of "no replies".
 10. Apify monthly hard-limit — cap reset was Aug 28. Verify reset occurred.
 11. `deliverability_gate.py` structural bug — PARTIALLY FIXED by
     `e113d09` (fail-closed). The local-CSV-only rollup issue persists.
 12. **67 total bounced addresses need suppression** — add all to
     `suppression.csv` to prevent re-bounce. Most are `hello@` pattern.
 13. **No briefing for 2026-09-09 yet** — latest is Sep 8. No Sep 9 KPI yet.
...
### Today's refresh (2026-09-08 11:03 EAT, Tue — daily)

- Re-read `outreach/data/gate-status.json` (08:03 UTC today — **FRESH**), `outreach/data/deliverability-snapshots/resend-7d-summary-2026-09-08.json` (08:03 UTC today — **FRESH**), `outreach/data/deliverability-status.json` (08:03 UTC today — **FRESH**), `outreach/data/crm-reconciliation-2026-09-08.json` (08:02 UTC today — **FRESH**), `outreach/data/experiment-status.json` (current), `outreach/data/rate-usage.json` (Sep 7: 24 sends), `security/dependency-audit-2026-09-07.md` (latest), `decisions.md`, `risks.md`.
- P0 launch state: **unchanged from 2026-08-04** — all P0 items closed.
  Launch happened 2026-07-22; we are now **48 days post-launch**.
  **No P0 launch escalation active.**
- **🚨🚨 GATE-STATE DELTA VS Sep 7 12:38 EAT REFRESH (~22h ago):**
  - `gate-status.json` (08:03 UTC today — **FRESH**):
    **`gate: pullback`**, `cap: 30`, `bounce_rate: 31.40%` (65/207),
    `spam: 0.0%`, `deliverability_status: fail`.
    **GATE REMAINS IN PULLBACK — Day 6.** Pipeline is THROTTLED to 30 sends/day.
  - **Fresh live API snapshot** (`resend-7d-summary-2026-09-08.json`, 08:03 UTC today — **FRESH**):
    **207 sends, 65 bounces**, **31.40% bounce rate**, 0 spam, 67.63% delivery.
    **🚨🚨 BOUNCE RATE EXPLODED: 18.01% → 31.40%.** Sends grew 161→207 (+46) while
    bounces surged 29→65 (+36 new bounces). **57 of 65 bounces are `hello@` pattern
    addresses** — enrichment source quality is the root cause.
    Bounces from Sep 2 OSM batch still in 7d window (age out ~Sep 9).
    **This is the worst bounce rate since launch.**
  - `deliverability-status.json` (08:03 UTC today — **FRESH**): `status: fail`,
    `bounce_rate: 31.40%` (65/207). Seed-inbox: **6/163 total** (up from 6/159),
    **156 awaiting** folder report (up from 152), 0 Promotions, 0 Spam.
    Still need 2 more Primary flips to graduate to `pass`.
  - CRM reconciliation (08:02 UTC today — **FRESH**): **324 rows** (UP from 272),
    **1 malformed** (DOWN from 115 — reconciliation now reading live outreach-log.csv
    as canonical; the PX-/OSM-/A-prefixed ID issue from Sep 7 has resolved).
    323 clean rows, 306 sent, 17 send_failed. Touch distribution: all t1.
    Rebuild NOT paused. **CRM is healthy.**
  - Experiment status: **A=77/1/1.30% (weight=1.0).**
    **B=77/0/0% (w=1.0), C=77/0/0% (w=1.0), D=76/0/0% (w=1.0)** —
    ...
    **All active variants should be killed per experiment rules. No working copy remains.**
    A weight is still 1.0 — A should also be killed (77 sends, 1.30% < 2% kill threshold).
  - `rate-usage.json` (Sep 7): 24 sends, 24 API calls. Sep 8 data not yet available.
    Pipeline active under pullback cap.
  - Git: **1 new commit** since Sep 7: `9b63198` (checklist daily refresh).
  - Dependency audit: **Sep 7 audit exists** (`security/dependency-audit-2026-09-07.md`).
    1c/29h/20m/6l = **56 total** (+3 from last week: +2 high, +1 moderate).
    New: `postcss` path traversal (high), +2 others. `node-tar` critical still no upstream fix.
  - Briefing: **No briefing for 2026-09-08 yet** — latest briefing is Sep 6.
  - KPI: Latest KPI is `2026-09-07.md` — no Sep 8 KPI yet.
- **Key changes since Sep 7:**
  1. **🚨🚨 BOUNCE RATE EXPLODED 18.01% → 31.40%.** +36 new bounces (29→65) vs +46 new sends (161→207).
     **57/65 bounces are `hello@` pattern addresses.** Enrichment source quality is the root cause.
     **Worst bounce rate since launch.** Bounces age out ~Sep 9 which should help dramatically.
  2. **✅ CRM malformed rows dropped 115 → 1.** Reconciliation now reading live outreach-log.csv
     as canonical. 324 total rows, 323 clean, 306 sent. CRM is healthy.
  3. **🚨 ALL experiment variants past kill threshold.** A=77/1/1.30% (below 2% kill).
     B=77/0/0%, C=77/0/0%, D=76/0/0%. **No working copy remains.**
     **Davie action needed: prepare new copy variants immediately.**
  4. Seed-inbox grew 159 → 163 but no new Primary flips.
  5. Pipeline active: 24 sends on Sep 7 under pullback cap.
  6. 1 new git commit (checklist refresh only).
  7. Dependency audit ran Sep 7: 56 total (+3). New `postcss` high vulnerability.
- **Carry-over escalation candidates (updated from Sep 7 refresh):**
  1. **🚨🚨 CRITICAL — Gate `pullback/cap=30`, 31.40% bounce (65/207). Day 6.**
     Bounce rate EXPLODED. 57/65 bounces are `hello@` pattern addresses.
     **Davie action needed URGENTLY:** (a) suppress ALL 65 bounced addresses in
     `suppression.csv`, (b) STOP sending to `hello@` pattern addresses immediately,
     (c) pause OSM-sourced prospect sends until enrichment quality is verified,
     (d) investigate/switch Apify enrichment source — `hello@` pattern is clearly
     generating invalid addresses, (e) fix `deliverability_gate.py` to read live API directly.
     **Bounces age out ~Sep 9 — rate should drop significantly then.**
     **THIS IS NOW A 48h+ ESCALATION ITEM — bounce rate has worsened for 2 consecutive days.**
  2. **🚨 CRITICAL — ALL experiment variants past kill threshold.**
     A=77/1/1.30% (below 2% kill), B=77/0/0%, C=77/0/0%, D=76/0/0%.
     **No working copy remains. Davie must prepare new copy variants immediately.**
     1 reply in 307 lifetime sends (0.33% overall).
  3. **Gate-bypass event 2026-08-09 — now ~30.4 days old, ~28 days past
     "needs decision" threshold.** 17 prospect IDs on T2 pause.
  4. Seed-inbox: 6/163 total, 156 awaiting. Need 2 more Primary flips
     to graduate to `pass`. Follow-ups remain paused (6th day).
  5. Dependency vulns: 1c/29h/20m/6l = 56. **+3 since last audit.**
     New `postcss` path traversal (high). `node-tar` critical still no upstream fix.
  6. Dev/prod isolation gap — open since 2026-08-04.
  7. Lana Hill reply — now **40 days** in `human_review_priority`,
     26 days past hard escalation threshold.
  8. `folder-nudge-DRAFTS-2026-08-11.md` — now 28 days old.
  9. imapclient install + IMAP poller — 48 days of "no replies".
 10. Apify monthly hard-limit — cap reset was Aug 28. Verify reset occurred.
 11. `deliverability_gate.py` structural bug — PARTIALLY FIXED by
     `e113d09` (fail-closed). The local-CSV-only rollup issue persists.
 12. **65 total bounced addresses need suppression** — add all to
     `suppression.csv` to prevent re-bounce. 57 are `hello@` pattern.
 13. **No briefing for 2026-09-08 yet** — latest is Sep 6. No Sep 8 KPI yet.
- Confirmed no new P0 launch-critical items. No new decisions logged
  since 2026-08-19 Growth/Scaling Autonomy carve-out.
- Per skill rules (semi-autonomy), no deployment, publishing, or
  "marked launched" actions taken.
